'''
Helper script to create several launch commands that extract the logits of a given model on a dataset folder. These are later used for faster evaluation of uncertainty metrics.
...
samples=20000 # Number of samples to be used to estimate calibration error


def get_done_filename(trg_dataset_, scene_, cond_, model_arch_, root_exp_dir_, samples):
    ...
    num_samples = f'num_samples_{samples}'
