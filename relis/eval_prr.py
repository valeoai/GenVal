...
class SolverOps:
    ...
    def eval_prr(self):
        ...
        if 'Mask' in self.args.model_arch:
            norm_logits = True # Maskformer logits are already normalized (into probs)
        else:
            norm_logits = False
        prr = uncertainty_ops.prediction_rejection_ratio(all_labels, all_logits, self.args.confidence_metric, norm_logits=norm_logits)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--src_dataset", type=str, default='Cityscapes',
                        help="Which source dataset to start from {Cityscapes, GTA, ADE20K}")
